...
@section('course')
    ...
    <div class="md:w-3/4 mx-auto my-10">
        ...
        <form method="POST" action="/course/{{$course->id}}/image" enctype="multipart/form-data">
            ...
            <button class="btn btn-secondary w-1/2 md:w-1/6">Change image</button>
        </form>
        <!-- Edit info -->
        <form method="POST" action="/course/{{$course->id}}">
            ...
            <textarea placeholder="Description..." name="description" class="textarea textarea-bordered w-full max-w-xs" cols="30" rows="5">{{$course->description}}
            </textarea><br>
            <button class="btn btn-primary my-1 w-1/2 md:w-1/6">Change</button>
        </form>
        <!-- Deleting course -->
        <h3 class="text-3xl my-2 ">Delete course</h3>
        <button class="btn btn-outline btn-error w-1/2 md:w-1/6" onclick="delete_modal.showModal()">Delete course</button>
